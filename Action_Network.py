...
class Action_Network:
    def __init__(self, sess, num_states, num_actions, seed_n, layer_size_A1, layer_size_A2, learning_rate_A, action_scale = 1):
        
        self.sess = sess

        #actor network model parameters:
        self.W1_a, self.B1_a, self.W2_a, self.B2_a, self.W3_a, self.B3_a,\
        self.actor_state_in, self.actor_model = self.create_actor_net(num_states, num_actions, layer_size_A1, layer_size_A2)

        #target actor network model parameters:
        self.t_W1_a, self.t_B1_a, self.t_W2_a, self.t_B2_a, self.t_W3_a, self.t_B3_a,\
        self.t_actor_state_in, self.t_actor_model = self.create_actor_net(num_states, num_actions, layer_size_A1, layer_size_A2)

        # action network training by calculating gradients
        ...
        self.optimizer = tf.train.AdamOptimizer(learning_rate_A).apply_gradients(zip(self.parameters_gradients,self.actor_parameters))  
    ...
    def create_actor_net(self, num_states=4, num_actions=1, N_HIDDEN_1=400, N_HIDDEN_2=300):
